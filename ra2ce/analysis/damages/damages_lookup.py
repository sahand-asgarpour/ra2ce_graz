...
class LookUp:
    ...
    def get_max_damages_osd(
        pricelevel_correction: float, gdp_per_capita_correction: float
    ):
        ...
        if (
            gdp_per_capita_correction > 0 or not np.isnan(gdp_per_capita_correction)
        ) or (pricelevel_correction > 0 or not np.isnan(pricelevel_correction)):
